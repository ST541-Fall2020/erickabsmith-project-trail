...
#' subdivided by the original function which uses rsample, the sequential 
#' function which doesn't, and the parallel function which mirrors the
#' sequential but uses furrr instead of purrr.
#' 
...
#' compare_bootstrap_prop_means(original_dat = data.frame(x = rnorm(10), prop=rbeta(10,2,2)))
#' future_get_bootstrap_prop_means(original_dat = data.frame(x = rnorm(10), prop=rbeta(10,2,2)))
